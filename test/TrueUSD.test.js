...
const OldTrueUSD = artifacts.require("OldTrueUSD")
...
contract('TrueUSD', function (accounts) {
    ...
    describe('--TrueUSD Tests: chaining 3 contracts where the first is the previous deployment--', function () {
        const _ = accounts[0]
        const owners = [accounts[1], accounts[2], accounts[3]]
        const oneHundreds = [accounts[4], accounts[5], accounts[6]]
        const anotherAccounts = [accounts[7], accounts[8], accounts[9]]
        ...
        beforeEach(async function () {
            ...
            this.tokens[0] = await OldTrueUSD.new({ from: owners[0] })
